...
const phaseTools = [
  ["carton-count", "Carton Count Calculator", "Calculate how many cartons are needed from required units and units per carton, including the quantity in a partial final carton.", [["units","Units required","125","count"],["perCarton","Units per carton","24","count"]], "Cartons = round up (units required ÷ units per carton)", "For 125 units at 24 units per carton, plan 6 cartons with 5 units in the final carton.", ["case-pack","master-carton-dimensions","master-carton-weight"], "/guides/master-carton-planning.html"],
  ["case-pack", "Case Pack Calculator", "Convert case quantities and loose reserve units into a total unit plan.", [["cases","Sealed case quantity","12","count"],["unitsPerCase","Units per case","24","count"],["reserve","Loose reserve units","6","count"]], "Total units = cases × units per case + reserve units", "Twelve cases of 24 plus 6 reserve units produce a 294-unit availability plan.", ["carton-count","master-carton-dimensions"], "/reference/master-carton-terms.html"],
...
guides.push(
  ...
  {
    slug: "master-carton-planning", title: "Master Carton Planning Guide", reviewed: "August 13, 2026", modified: "2026-08-13", description: "Plan unit layout, internal dimensions, packed weight, closure, and verification for master cartons.", intro: "A master carton plan connects the packed unit, repeatable layout, carton strength, weight, and downstream handling.",
...
const guideDepth = {
  ...
  "master-carton-planning": {
    ...
    sectionNotes: [
      "Keep the measurement record with the SKU or pack revision, measurement unit, allowed orientation, sample identity, and date so a later carton plan can be reproduced.",
      "Record the selected columns × rows × layers arrangement with gap, divider, and wall-clearance assumptions. If another arrangement is rejected, note the fit or handling constraint that ruled it out.",
      "Use the approved case pack in the Master Carton Weight Calculator, then measure finished external dimensions for Carton Cube. Internal design size and external shipping cube are separate records.",
      "Release the pack instruction only after the full prototype, closure method, external measurements, gross weight, and handling observations agree with the recorded design."
    ],
    closeoutIntro: "Release the master-carton instruction only when the recorded layout matches a full physical prototype and the measured external size, gross weight, closure, and handling checks are complete.",
    evidence: [
      "Keep the packed-unit revision, measurement unit, allowed orientation, layout, gaps, clearance, prototype identity, external dimensions, gross weight, date, and reviewer together. This creates one traceable path from design inputs to the released carton instruction.",
      "Reopen the plan when the product or inner pack changes, when the carton or divider specification changes, or when production samples show movement, bulging, closure difficulty, damage, or weight outside the approved range.",
      "During review, compare normal production cartons and known exceptions with the recorded prototype. Revise the controlled instruction when an input or acceptance decision changes; do not silently overwrite the original design basis."
    ],
    note: "Use calculated internal dimensions to build a prototype, not as finished external shipping dimensions. Confirm the actual materials, handling path, and current customer or carrier requirements before release.",
...
references.push(
  ...
  {
    slug: "master-carton-terms", title: "Master Carton Terms", reviewed: "August 13, 2026", modified: "2026-08-13", description: "Define case pack, master carton layout, tare, cube, and packed-carton measurements.", intro: "Use the same master-carton terms across purchasing, packing, storage, and pallet planning.",
...
const referenceDepth = {
  ...
  "master-carton-terms": {
    ...
    maintenance: [
      "For each carton specification, record the packed-unit revision, case pack, allowed layout, measurement unit, effective date, and source drawing or physical sample. Preserve the original measured values before operational rounding.",
      "Link internal dimensions to the layout and prototype that established them. Record external dimensions and gross packed weight from the finished carton, and keep tare as a separate measured value.",
      "Supersede the record when the product, inner pack, carton, divider, closure, or case pack changes. Recheck dependent weight, cube, storage, and pallet calculations before releasing the revision."
    ],
    note: "Treat these definitions as a shared record structure. Verify changing dimensions, weights, material specifications, and external requirements in the current controlled source before operational use."
  },
...
function head({ file, title, description, type = "website", noindex = false, schema }) {
  ...
  const usesWorkflowLayoutFix = ["pack-instructions.html", "guides/writing-pack-instructions.html", "reference/pack-instruction-record-fields.html"].includes(file);
  const styleVersion = usesWorkflowLayoutFix ? "20260813-workflow-layout" : file.startsWith("tools/") ? "20260810-calculator-inputs" : usesReviewedMeta ? "20260810-review-ux" : "20260802-quality";
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
<main id="main">
  ...
  <section class="article-zone"><div class="page-shell article-shell"><article class="article-body">
    ...
    <h2 id="example">Worked example</h2><div class="example-block"><p>${tool.example}</p><p><strong>Next action:</strong> ${nextAction}</p></div>
...
function guideSectionNote(item, detail, heading, index) {
  return detail.sectionNotes?.[index] || `In ${item.title}, document the ${heading.toLowerCase()} choice, its measured basis, and any exception that changes the standard procedure.`;
}

function guideEvidence(item, detail) {
  if (detail.evidence) return detail.evidence.map((paragraph) => `<p>${paragraph}</p>`).join("");
  return `<p>The ${item.title} record should connect the initial requirement—${detail.prepare[0].replace(/\.$/, "").toLowerCase()}—to the released evidence that ${detail.closeout[0].replace(/\.$/, "").toLowerCase()}. Keep the ${item.title} inputs, sample identification, material or equipment revision, date, operator or reviewer role, and exception decision together so a later result can be compared on the same basis.</p><p>Reopen ${item.title} when ${detail.mistakes[0].replace(/\.$/, "").toLowerCase()} is observed, when the product or packaging specification changes, or when damage, rework, time, or consumption moves outside the accepted range. The ${item.title} owner should compare the new condition with the working scenario, repeat the relevant physical check, and issue a revised instruction rather than silently changing an input.</p><p>For periodic review, sample normal work as well as known exceptions. Confirm that the response to “${detail.decisions[0][0]}” still follows the recorded action: ${detail.decisions[0][1]} Retain the ${item.title} evidence long enough to explain inventory settings, cost changes, and any customer or carrier inquiry tied to the pack method.</p>`;
}

function referenceMaintenance(item, detail) {
  if (detail.maintenance) return detail.maintenance.map((paragraph) => `<p>${paragraph}</p>`).join("");
  return `<p>A working ${item.title} record should identify the source document or measurement, unit and scope, effective date, reviewer role, and the calculator or pack instruction that consumes the value. Start by ${detail.use[0].replace(/\.$/, "").toLowerCase()}, then preserve the unrounded or source value before any operational rounding or simplification.</p><p>Do not treat the glossary entry “${item.rows[0][0]}” as self-approving data. Link the ${item.title} entry to the applicable drawing, supplier specification, official method, measured sample, or controlled procedure. When ${detail.cautions[0].replace(/\.$/, "").toLowerCase()} becomes relevant, mark the old record superseded, update linked calculations, and recheck downstream fit, cost, inventory, or handling decisions.</p><p>The ${item.title} applied example shows the minimum audit trail: original inputs, intermediate relationship, displayed result, and the action it supports. A periodic ${item.title} review should also verify that the distinction between “${detail.differences[0][0]}” and its paired operating meaning remains clear to people entering data.</p>`;
}

function applyArticleOverrides(body, item, kind, guideDetail, referenceDetail) {
  let updated = body;
  if (kind === "Guides" && guideDetail.sectionNotes) {
    item.sections.forEach(([heading], index) => {
      const generic = `<p>In ${item.title}, document the ${heading.toLowerCase()} choice, its measured basis, and any exception that changes the standard procedure.</p>`;
      updated = updated.replace(generic, `<p>${guideSectionNote(item, guideDetail, heading, index)}</p>`);
    });
    updated = updated.replace(
      `<p>Close ${item.title} only after its physical result, controlled instruction, and recorded measurements agree with the decision criteria above.</p>`,
      `<p>${guideDetail.closeoutIntro}</p>`
    );
    updated = updated.replace(
      /<h2 id="evidence">[\s\S]*?<h2 id="checklist">/,
      `<h2 id="evidence">Evidence, ownership, and review triggers</h2>${guideEvidence(item, guideDetail)}<h2 id="checklist">`
    );
    updated = updated.replace(
      `<div class="caution"><strong>${item.title} planning note:</strong> validate the ${item.title} method with the actual product, materials, handling path, and current shipping requirements.</div>`,
      `<div class="caution"><strong>Prototype verification:</strong> ${guideDetail.note}</div>`
    );
  }
  if (kind === "Reference" && referenceDetail.maintenance) {
    updated = updated.replace(
      /<h2 id="maintenance">[\s\S]*?<h2 id="verification">/,
      `<h2 id="maintenance">Record structure and maintenance</h2>${referenceMaintenance(item, referenceDetail)}<h2 id="verification">`
    );
    updated = updated.replace(
      `<div class="caution"><strong>${item.title} reference note:</strong> recheck every changing ${item.title} value in the current supplier, carrier, marketplace, facility, or regulatory source before operational use.</div>`,
      `<div class="caution"><strong>Record note:</strong> ${referenceDetail.note}</div>`
    );
  }
  return updated;
}
...
function articlePage(item, kind) {
  ...
  const schema = {
    "@context": "https://schema.org",
    "@graph": [
      { "@type": "Article", headline: title, description: item.description, datePublished: "2026-07-26", dateModified: item.modified || (item.reviewed ? "2026-08-02" : "2026-07-26"), mainEntityOfPage: pageUrl(file), author: { "@type": "Organization", name: "Pack Prep Tools" }, publisher: { "@type": "Organization", name: "Pack Prep Tools" } },
      { "@type": "BreadcrumbList", itemListElement: [
        { "@type": "ListItem", position: 1, name: "Home", item: `${SITE}/` },
...
<section class="article-zone"><div class="page-shell article-shell"><article class="article-body">${finalBody}<ul class="related-register"><li><a href="${related}">Related calculator</a></li><li><a href="${crossDocument}">Related ${kind === "Guides" ? "reference" : "guide"}</a></li><li><a href="/${folder}.html">All ${folder}</a></li></ul><p class="meta-line">Last reviewed: ${item.reviewed || REVIEWED}</p></article></div></section></main>${footer()}`;
}
