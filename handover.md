...
## 2026-08-27 — 공격적 신규 workflow cluster 확장: Packaging Adhesive Application GO

### 시작 환경·동기화·실제 inventory

- 실제 작업 경로: `C:\Users\song\Documents\ChatGPT\packpreptools\repo` (경로를 사전 가정하지 않고 현재 workspace에서 remote를 확인해 선택). 상위 `packpreptools` 디렉터리는 별도의 빈 wrapper Git repository였고 실제 제품 checkout은 그 아래 `repo`였다.
- remote: `origin https://github.com/canghun13/packpreptools.git`; branch `main`; 시작 working tree clean.
- 시작 local HEAD: `bca2ffd84ae5041c8439e4afa6107d9218c28de0`; fetch 전 실제 remote main 및 `origin/main`: `7e0de008f8341f6e115e29e97ae7f8d1fb3e4df2`. 로컬이 2 commits behind이고 변경이 없어 `git pull --ff-only origin main`으로 안전하게 동기화했다. 조사·구현 기준 HEAD는 `7e0de008f8341f6e115e29e97ae7f8d1fb3e4df2`다.
- 시작 실제 public inventory: HTML 76, sitemap URL 75, Tool page 40 = Calculator 36 + Workflow Tool 4, Guide 14, Reference 12, hub/other 10. JavaScript 7. 활성 cluster는 일반 계산기 registry, Packaging Quality & Damage Control, Pack Instruction & Job Release다.
- authoritative source는 `scripts/generate-site.js`; 계산 로직은 `assets/calculators.js`; 생성된 HTML은 직접 편집하지 않았다. 검증 축은 content/responsive checks를 내장한 `scripts/qa.js`, `scripts/verify-calculators.js`, `scripts/verify-workflow-tools.js`다.
- 최근 production 상태인 calculator mobile input-description, Packaging Trial Decision Guide와 Pack Instruction tables의 scoped mobile layout, compact Last reviewed, Shipping Damage Rate defaults/Reset, Master Carton/Carton Count fixes, 두 workflow cluster, Pallet Utilization simple-grid/Pattern check를 baseline으로 고정했다.

### Underlying-problem exclusion inventory — 82 families

- 이전 기록 전체를 GO/implemented/HOLD/REJECT/MERGE/NO-GO 기준으로 거슬러 읽었다. 명칭이 아니라 workflow problem으로 합쳐 82개를 제외했다.
- 선행 50: Returns/reverse logistics; Purchasing/quote; Fulfillment accuracy/rework; Shipment consolidation; Sustainability; Automation; Changeover; Label-roll planning; Stretch film; Cut yield; Storage; Compression; Weight; Packing station; Industrial bags/liners/covers; Packaging version cutover; Point-of-use replenishment; Order mix/deadline; Strapping/edge protection; Kit availability; Partitions/dividers; Carton portfolio; Repack/overpack; Mailing tube; Label print setup; Pack Instruction/Job Release(implemented); Supplier handoff; Sample/proof approval; Label placement; Closure troubleshooting; Pack sequence; Scale verification; Dispatch exception; Photo evidence; Shipment documents; Artwork preflight; GS1 ID/hierarchy; Export marks; Parcel billing evidence; Irregular measurement; Dimension drift; FEFCO/dieline; Material receiving; Cold chain; Liquids; ESD; Accessible opening; Wooden crate/export; Packaging master data; Packaging Quality & Damage Control(implemented).
- 직전 32: Small-parts count-by-weight; Apparel assortment/prepack; Shelf-ready display; Security seal lifecycle; Moisture dry-pack; Heat-seal operating window; Shrink tunnel; Vacuum compression; Pouch fill/headspace; Put-wall/cubby; Pick tote; Dispatch staging lanes; Component lot genealogy; Component shelf-life; Tool/fixture readiness; Retention samples; Carton pre-erection; Bagging/sealing cell balance; Postal flats; VCI; Magnetic items; Cleanroom double-bag; Powder containment; Odor containment; Light protection; Spool/reel; Cable/hose coil; Nested separators; Surface interleave; Garment-on-hanger; Retail PDQ; Serialized insert collation.
- 이 때문에 예컨대 hot-melt 후보도 기존 **closure troubleshooting**을 다시 이름 붙인 것이 아니라, 증상 진단·결함 원인 분기와 분리된 **소모재 application 계획/측정/시간당 capacity/refill/output record**로만 평가했다.

### Discovery 40 families / 160 Tool hypotheses

표기: 각 Tool은 `이름[type] — q=likely query; I=input; L=logic; O=output; A=next action` 순서다. family 공통 열에 target user, repeat trigger, closest existing/delta를 기록했다. 따라서 모든 가설은 Tool name/type/target/problem/query/input/logic/output/action/repeat/closest/difference를 포함한다.

| # / genuinely new family와 실제 problem | target / repeat trigger / closest existing와 차이 | 최소 4 Tool hypotheses | rapid screen |
|---|---|---|---|
| 1. Hot-melt adhesive application — bead intent를 실제 소비·capacity·refill·output으로 연결 | 포장 셀 운영자·소규모 제조; SKU/run/pattern/equipment가 바뀔 때; Tape Usage/Closure Troubleshooting과 달리 adhesive consumable flow를 수치화 | Bead Volume **[calculator]** — q=`hot melt bead volume calculator`; I=diameter,length,paths,density; L=cylinder volume×density; O=mL,g/pack; A=trial. Batch Requirement **[planner]** — q=`hot melt adhesive consumption calculator`; I=packs,g/pack,allowance,container; L=batch mass+ceil; O=kg,containers; A=stage. Intermittent Savings **[comparator]** — q=`hot melt dotting savings calculator`; I=continuous,on/off,run; L=duty-cycle comparison; O=g/pack,kg delta; A=controlled trial. Melt-rate Capacity **[checker]** — q=`hot melt melt rate calculator`; I=g/pack,rate,allowance,melt rate; L=hourly demand/margin; O=utilization,flag; A=verify capacity. Refill Planner **[planner]** — q=`hot melt tank refill frequency`; I=tank,start,reserve,use,run; L=inventory depletion; O=time/refills; A=schedule check. Output Calibration **[checker]** — q=`hot melt glue output calculation g/min`; I=mass,time,nozzles,target,tolerance; L=flow+variance; O=g/min/nozzle,flag; A=inspect/retest | **FINALIST / GO**, strong 6 |
| 2. Gift-presentation wrapping — gift box/bag/wrap/ribbon components를 한 job으로 준비 | 소매점·선물 포장자; item/order/theme 변경; material calculators와 달리 presentation configuration | Wrap Sheet **[calculator]** — q=`gift wrap size calculator`; I=box LWH,overlap; L=net layout; O=sheet size; A=cut. Ribbon Length **[calculator]** — q=`ribbon length calculator gift box`; I=box,wrap style,bow; L=path+allowance; O=length; A=cut. Gift Bag Fit **[checker]** — q=`gift bag size calculator`; I=item,bag,gusset; L=usable-volume constraints; O=fit flags; A=size up. Tissue Count **[planner]** — q=`tissue paper calculator gift bag`; I=bag/item/fullness; L=coverage/yield; O=sheets; A=stage | exact free suite already exists; REJECT |
| 3. Package center of gravity/manual handling geometry — asymmetric pack의 balance point와 lift 표시 | pack engineer·warehouse lead; product position/configuration 변경; Weight/Box Fit과 달리 moments와 handle relation | CG Axis **[calculator]** — q=`package center of gravity calculator`; I=item masses,positions; L=weighted moments; O=x/y/z CG; A=reposition. Support Polygon **[checker]** — q=`center of gravity stability checker box`; I=base,CG projection; L=inside-margin test; O=edge margins; A=widen/reposition. Lift-point Balance **[planner]** — q=`lifting point balance calculator`; I=CG,strap/handle positions; L=reaction split; O=load shares; A=move points. CG Mark **[generator]** — q=`center of gravity shipping label template`; I=dimensions,CG coordinates; L=scale placement; O=printable mark; A=apply/verify | safety/structural interpretation and weak packaging SERP; REJECT |
| 4. Pick-and-pluck foam insert layout — cavity grid를 product footprint와 reserve에 맞춤 | case builder·small shipper; product/case/foam grid 변경; Partitions/Cut Yield와 달리 removable-cell cavity | Cavity Grid **[visualizer]** — q=`pick pluck foam layout calculator`; I=case,grid pitch,product; L=cell rounding; O=SVG cavity; A=pluck trial. Wall Thickness **[checker]** — q=`foam insert wall thickness calculator`; I=cavities,spacing,edge; L=min-gap scan; O=weak walls; A=re-space. Layer Planner **[planner]** — q=`foam insert layer thickness calculator`; I=item height,lid/base layers; L=stack-up; O=layer plan; A=select sheets. Multi-item Layout **[builder]** — q=`foam case layout tool`; I=footprints,rotations; L=deterministic grid placement; O=cell map; A=mark foam | partitions/cut-yield overlap; 3D/protection validation; REJECT |
| 5. Floor-loaded container/trailer planning — cartons를 pallet 없이 floor에 배치 | exporter·warehouse planner; carton/container/mix 변경; Pallet tools와 달리 floor-load orientations and axle/weight limits | Floor Grid **[visualizer]** — q=`container floor loading calculator`; I=container/carton dims; L=orientation grids; O=count/map; A=load plan. Mixed Load **[planner]** — q=`mixed carton container loading`; I=SKU dims/qty; L=heuristic packing; O=sequence/utilization; A=stage. Weight Zone **[checker]** — q=`container weight distribution calculator`; I=positions,masses,zones; L=zone sums; O=imbalance; A=reorder. Load Comparison **[comparator]** — q=`20ft 40ft container loading calculator`; I=cartons,container options; L=fit/cost-neutral capacity; O=counts/remainder; A=select | free 3D tools outperform static simple engine; transport-safety scope; REJECT |
| 6. Product sleeve/belly-band fitting — band blank와 overlap을 product stack에 맞춤 | DTC seller·converter; product/stack/overlap 변경; FEFCO/Dieline 제외 영역과 달리 simple wrap band | Sleeve Blank **[calculator]** — q=`belly band size calculator packaging`; I=wrapped perimeter,width,overlap; L=perimeter+overlap; O=blank W×L; A=prototype. Overlap **[checker]** — q=`paper sleeve overlap calculator`; I=blank,product,minimum; L=available overlap; O=margin; A=resize. Sheet Yield **[planner]** — q=`belly band print sheet yield`; I=sheet,blank,gap,qty; L=2D grids; O=pieces/sheets; A=order. Stack Sleeve **[visualizer]** — q=`product sleeve fit template`; I=item count/dims,orientation; L=bounding box+band path; O=diagram; A=test | BoxTemplate free template; one geometry split into modes; REJECT |
| 7. Nested-product stack height — tapered/nesting items의 compressed stack plan | housewares/industrial packer; item/count/nesting depth 변경; Compression/Case Pack과 달리 incremental nested pitch | Nest Pitch **[calculator]** — q=`nested stack height calculator`; I=single height,nested height/sample count; L=incremental pitch; O=pitch; A=record. Stack Height **[planner]** — q=`nesting product stack height`; I=first height,pitch,count; L=first+(n−1)pitch; O=height; A=choose count. Count-to-Limit **[checker]** — q=`how many nested cups fit box`; I=limit,first,pitch; L=floor inverse; O=count/headroom; A=set pack. Sample Reconciliation **[validator]** — q=`nesting ratio calculator packaging`; I=measured sample,predicted; L=variance; O=tolerance flag; A=remeasure | strong only 2–3; Zonos nesting feature but weak standalone search; HOLD |
| 8. Telescoping two-piece enclosure fit — base/lid overlap과 assembled height 계산 | custom carton user; product/base/lid 변경; Box Size와 달리 two-piece telescoping stack-up | Base/Lid Size **[calculator]** — q=`telescoping box size calculator`; I=product,clearance,board; L=inside/outside offsets; O=two blanks; A=prototype. Overlap Depth **[checker]** — q=`telescoping box overlap calculator`; I=lid/base depths,height; L=coverage; O=overlap/gap; A=adjust. Height Range **[planner]** — q=`adjustable telescopic box height`; I=component depth/min overlap; L=range; O=min/max pack height; A=select set. Component Match **[validator]** — q=`telescopic carton lid base fit`; I=base/lid dimensions,tolerance; L=clearance comparisons; O=fit flags; A=sample | Dieline/carton-fit adjacency; board allowances need expert data; REJECT |
| 9. Protective cap/plug allocation — product ends/openings별 reusable caps를 staging | industrial parts packer; part/end/qty 변경; Kit Availability와 달리 interface coverage | End Count **[planner]** — q=`protective cap quantity calculator`; I=parts,ends/cap,spares; L=multiply+allowance; O=cap qty; A=stage. Size Match **[selector]** — q=`plastic cap plug size selector`; I=ID/OD/interface,tolerance; L=user-entered range match; O=candidates; A=sample. Coverage Audit **[checker]** — q=`thread protector checklist tool`; I=required/assigned locations; L=set difference; O=missing/duplicate; A=correct. Reuse Cycle **[calculator]** — q=`reusable protective caps quantity`; I=daily jobs,return rate,cycle time; L=circulating stock; O=pool size; A=stock | vendor catalog dependency; kit/reuse logic overlap; REJECT |
| 10. Pallet-collar configuration — collar height/boards/layers를 load에 맞춤 | small warehouse/exporter; load/collar size 변경; Pallet Height와 달리 modular collar increments | Collar Count **[calculator]** — q=`pallet collar height calculator`; I=load height,base/top,collar height; L=ceil layers; O=collars/assembled height; A=stage. Volume **[calculator]** — q=`pallet collar volume calculator`; I=inside dims,count; L=volume; O=capacity; A=compare. Divider Grid **[planner]** — q=`pallet collar divider layout`; I=items/grid/walls; L=cell allocation; O=divider plan; A=assemble. Option Compare **[comparator]** — q=`pallet collar vs box capacity`; I=collar options,load; L=height/volume/remainder; O=fit table; A=choose | Pallet Height/Partitions combination; 4th not independent; REJECT |
| 11. Slip-sheet pull-tab/load preparation — tab orientation과 load footprint를 push-pull handling에 맞춤 | slip-sheet user; load/equipment/tab change; Pallet tools와 달리 sheet/tab geometry | Sheet Size **[calculator]** — q=`slip sheet size calculator`; I=load footprint,edge allowance; L=offsets; O=sheet dimensions; A=order. Tab Orientation **[selector]** — q=`slip sheet pull tab orientation`; I=handling direction/transfer count; L=rule map; O=tab sides; A=specify. Tab Clearance **[checker]** — q=`slip sheet tab clearance`; I=load,tab,fork/equipment envelope; L=clearance; O=conflicts; A=test. Load Diagram **[visualizer]** — q=`slip sheet pallet layout`; I=footprint/tab orientation; L=SVG plan; O=handling map; A=brief operator | equipment-specific and safe-handling responsibility; weak free-tool intent; REJECT |
| 12. Product anchoring/block positioning — crate/box 내 block 위치와 clearance 기록 | heavy-product pack planner; item/base/block changes; Wooden Crate excluded, but generic anchoring geometry considered | Block Coordinate **[builder]** — q=`shipping crate blocking layout`; I=base,item,block dims; L=coordinate placement; O=plan view; A=mark base. Clearance **[checker]** — q=`blocking bracing clearance calculator`; I=item/block/wall gaps; L=collision checks; O=flags; A=reposition. Fastener Map **[generator]** — q=`crate blocking fastener layout`; I=approved spacing/edges; L=user-rule placement; O=mark list; A=review. Scenario Compare **[comparator]** — q=`shipping restraint layout comparison`; I=two layouts; L=space/count delta; O=comparison; A=prototype | structural/transport safety and wooden-crate exclusion; REJECT |
| 13. Shipping skid/base layout — non-pallet machinery base footprint와 runner spacing 계획 | machinery shipper; machine/base/fork entry 변경; Pallet Fit과 달리 custom skid parts | Deck Size **[calculator]** — q=`shipping skid size calculator`; I=product footprint,clearance; L=bounding offsets; O=deck size; A=draw. Runner Spacing **[planner]** — q=`skid runner spacing calculator`; I=approved support points/fork width; L=geometry; O=positions; A=prototype. Fork Entry **[checker]** — q=`forklift entry clearance skid`; I=runner/opening/fork envelope; L=clearance; O=fit flag; A=verify truck. Cut List **[generator]** — q=`shipping skid cut list calculator`; I=deck/runners/member sizes; L=piece aggregation; O=cut list; A=source lumber | structural safety and Crate/Export overlap; REJECT |
| 14. Corrugated hand-hole/handle placement — manual handling opening 위치와 남은 board geometry | corrugated designer; carton/load/handle location 변경; Box Size와 달리 handle placement | Placement **[visualizer]** — q=`corrugated hand hole placement`; I=panel,opening,offsets; L=scaled SVG; O=diagram; A=sample. Edge Distance **[checker]** — q=`box hand hole edge distance`; I=user minimums,opening,score edges; L=clearance; O=flags; A=move opening. Grip Pair **[planner]** — q=`carton handle spacing calculator`; I=carton width,CG,hand spacing; L=symmetry; O=coordinates; A=mark. Open Area **[calculator]** — q=`hand hole area calculator`; I=shape/dims/count; L=shape area; O=removed area%; A=engineering review | ISO/ergonomics/structural safety; article intent dominant; FINALIST then REJECT |
| 15. Molded-pulp tray cavity utilization — cavity pitch와 sheet/tray footprint planning | molded-fiber buyer/designer; product/count/pitch change; Partitions differs by molded cavities | Cavity Pitch **[planner]** — q=`molded pulp tray cavity layout`; I=tray,item,gaps; L=grid orientation; O=cavity count/map; A=prototype. Nest Stack **[calculator]** — q=`molded pulp tray stack height`; I=first height,nesting pitch,count; L=nested stack; O=height; A=plan case. Material Footprint **[calculator]** — q=`molded fiber tray area calculator`; I=tray flange/cavity footprint; L=bounding area; O=sheet area; A=quote basis. Fit Comparator **[comparator]** — q=`molded pulp tray cavity size`; I=item envelope,two cavity options; L=clearance; O=margins; A=sample | custom tooling/engineering; Nest/partition overlap; REJECT |
| 16. Thermoformed blister/clamshell cavity planning — formed cavity draft/flange/seal-area envelope | packaging developer; product/cavity/tooling change; existing fit tools omit formed-web geometry | Cavity Envelope **[calculator]** — q=`thermoform cavity size calculator`; I=product,clearance,draft,depth; L=offset geometry; O=top/bottom envelope; A=prototype. Flange Width **[checker]** — q=`blister seal flange width`; I=tool outline,cavity,user minimum; L=perimeter clearance; O=minimum flange; A=review. Web Pitch **[planner]** — q=`thermoforming web pitch calculator`; I=cavity,lanes,margins; L=repeat layout; O=pitch/utilization; A=quote. Nest Height **[calculator]** — q=`thermoformed tray nesting height`; I=first/pitch/count; L=nested stack; O=shipping height; A=case plan | engineering/manufacturing upstream; material-specific validation; REJECT |
| 17. Carton window-film patch planning — die-cut window를 덮는 patch 크기와 roll use | folding-carton converter; window/patch/repeat/job changes; Label/Cut Yield differs by interior patch | Patch Size **[calculator]** — q=`carton window patch size calculator`; I=window,seal overlap; L=offset rectangle; O=patch dims/area; A=spec. Roll Usage **[planner]** — q=`window patch film consumption`; I=patch,pitch,qty,waste; L=length+area; O=roll length/area; A=stage. Coverage **[checker]** — q=`window patch overlap checker`; I=window/patch offsets,user minimum; L=edge margins; O=flags; A=recenter. Layout **[visualizer]** — q=`window patch layout tool`; I=panel/window/patch; L=SVG overlay; O=diagram; A=approve sample | converter niche; patch sizing/roll use same geometry, low site fit; REJECT |
| 18. Corrugated ventilation/open-area planning — vent holes의 총 open area와 placement conflict | produce/industrial carton designer; hole pattern/panel changes; not general box volume | Open Area **[calculator]** — q=`corrugated box vent area calculator`; I=shape,dims,count,panel; L=area ratio; O=open%; A=compare target. Pattern **[visualizer]** — q=`carton ventilation hole pattern`; I=panel,rows,columns,offsets; L=SVG grid; O=layout; A=prototype. Clearance **[checker]** — q=`vent hole score clearance box`; I=holes,scores,user clearances; L=collision test; O=flags; A=move. Pattern Compare **[comparator]** — q=`carton vent pattern comparison`; I=two patterns; L=open area/count/clearance; O=table; A=trial | structural/cooling performance cannot be inferred; article/CAD intent; REJECT |
| 19. Grease/oil barrier wrap coverage — coated sheet/wrap coverage and overlap for oily parts | industrial packer; part/sheet/overlap/job changes; Bubble Wrap differs by barrier wrap pattern | Wrap Size **[calculator]** — q=`greaseproof paper wrap size calculator`; I=item dims,fold,overlap; L=wrap net; O=sheet size; A=cut. Sheet Count **[planner]** — q=`oil barrier paper quantity calculator`; I=sheet yield,qty,waste; L=ceil batches; O=sheets/reams; A=stage. Overlap **[checker]** — q=`barrier wrap overlap checker`; I=sheet/item/min overlap; L=coverage margins; O=gap flag; A=size up. Pattern Compare **[comparator]** — q=`wrap pattern material comparison`; I=two wrap methods; L=area/folds; O=material delta; A=trial | material persona of existing wrap/coverage and VCI adjacency; REJECT |
| 20. Water-activated tape application planning — strip length, water use, roll/run, dispenser pace | carton sealing operator; carton/pattern/run/dispenser changes; Tape Usage lacks WAT activation/consumption | Strip Length **[calculator]** — q=`water activated tape length calculator`; I=carton seam,overhang,strips; L=path; O=length/carton; A=set cut. Roll Yield **[planner]** — q=`gummed tape roll yield calculator`; I=roll/strip/run/waste; L=floor/ceil; O=cartons/rolls; A=stage. Water Demand **[calculator]** — q=`water activated tape dispenser water usage`; I=manufacturer/user mL/strip,strips; L=multiply; O=water/run; A=refill plan. Pace **[checker]** — q=`gummed tape dispenser capacity`; I=strips/min,pack rate,verified capacity; L=demand margin; O=utilization; A=trial | core strip/roll is existing Tape Usage; water rate vendor-specific; strong 2; REJECT |
| 21. Carton staple/stitch closure pattern — user-approved spacing에서 fastener count와 pattern | industrial carton operator; seam/spacing/carton/run changes; Closure Troubleshooting differs from setup count | Stitch Count **[calculator]** — q=`carton stitching spacing calculator`; I=seam,edge offset,max spacing; L=ceil intervals; O=count/positions; A=set approved pattern. Pattern **[visualizer]** — q=`corrugated box staple pattern`; I=panels/positions; L=SVG marks; O=diagram; A=work instruction. Run Quantity **[planner]** — q=`carton staples quantity calculator`; I=count/carton,qty,waste; L=multiply/box ceil; O=staples; A=stage. Pattern Audit **[checker]** — q=`carton staple spacing checker`; I=observed positions,user tolerance; L=gaps/edge compare; O=flags; A=inspect | safety/board/equipment rules; count and audit one pattern engine; REJECT |
| 22. Carton glue-tab joint geometry — manufacturer joint overlap와 adhesive area | corrugated/folding carton converter; blank/tab/bead changes; Dieline/closure excluded family adjacent | Tab Width **[calculator]** — q=`carton glue flap width calculator`; I=panel/tab/overlap; L=geometry; O=joint dimensions; A=draft. Bond Area **[calculator]** — q=`carton glue area calculator`; I=tab,bead/coverage; L=area; O=nominal area; A=material plan. Joint **[visualizer]** — q=`carton manufacturer joint diagram tool`; I=panel/tab/bead offsets; L=SVG; O=joint view; A=sample. Clearance **[checker]** — q=`glue flap score clearance`; I=tab,score,application offsets; L=margin; O=conflicts; A=adjust | dieline and hot-melt overlap; engineering assumptions; REJECT |
| 23. Paperboard score/fold allowance — crease offsets를 folded dimensions에 변환 | paperboard prototype maker; caliper/fold/blank change; FEFCO/Dieline family already excluded but distinct converting math tested | Fold Allowance **[calculator]** — q=`paperboard fold allowance calculator`; I=caliper,angle,factor; L=bend allowance model; O=flat length; A=prototype. Score Position **[planner]** — q=`box score line calculator`; I=finished panels,allowances; L=cumulative offsets; O=score coordinates; A=mark blank. Fold Stack **[checker]** — q=`paperboard fold thickness calculator`; I=layers/caliper/gaps; L=stack-up; O=closed thickness; A=verify fit. Blank Compare **[comparator]** — q=`paperboard blank allowance comparison`; I=two allowance models; L=dimension delta; O=score shifts; A=sample | same dieline underlying problem; material-model risk; excluded/MERGE |
| 24. Corrugated blank caliper stack/feeder planning — flat blanks의 stack height와 replenishment cadence | case erector/print feeder operator; caliper/run/magazine changes; Carton Pre-erection excluded but flat-blank feeder distinct evaluated | Stack Height **[calculator]** — q=`corrugated sheets stack height calculator`; I=caliper,count; L=multiply; O=height; A=stage. Magazine Count **[calculator]** — q=`case erector magazine capacity cartons`; I=usable length,caliper,compression factor; L=floor; O=blanks/load; A=set batch. Refill Cadence **[planner]** — q=`carton magazine refill frequency`; I=capacity,rate,reserve; L=depletion; O=minutes/refills; A=schedule. Caliper Variance **[checker]** — q=`corrugated board caliper stack variance`; I=sample stack/count,target tolerance; L=mean/variance; O=flag; A=remeasure | pre-erection buffer/changeover/replenishment overlap; REJECT |
| 25. Corrugated flute-direction blank orientation — grain/flute direction을 load/score/orientation record에 연결 | carton designer/operator; SKU/blank/stack direction changes; no current directional tool | Orientation **[selector]** — q=`corrugated flute direction box`; I=load axis,score axis,handling; L=user-approved rule map; O=orientation; A=mark drawing. Arrow **[generator]** — q=`flute direction symbol template`; I=panel/view/direction; L=SVG annotation; O=print mark; A=attach spec. Rotation Audit **[checker]** — q=`corrugated blank orientation checker`; I=required vs observed direction; L=match; O=pass/flag; A=segregate. Sheet Yield **[comparator]** — q=`corrugated flute direction sheet layout`; I=sheet/blank,allowed rotations; L=orientation-restricted grid; O=yield; A=choose layout | rules need engineering; Cut Yield/Dieline overlap; strong intent weak; REJECT |
| 26. Flexographic ink coverage/consumption — artwork coverage와 press volume에서 job ink를 계획 | packaging printer; artwork/repeat/anilox/job changes; Artwork Preflight differs from consumable mass | Coverage **[calculator]** — q=`flexo ink consumption calculator`; I=print area,coverage,volume,qty,waste; L=area×deposit; O=L/kg; A=stage. Color Split **[planner]** — q=`flexographic ink quantity per color`; I=color coverages/repeats; L=per-color aggregation; O=color quantities; A=mix. Job Compare **[comparator]** — q=`ink coverage cost comparison flexo`; I=two artworks/coverage/cost; L=consumption delta; O=cost/material; A=quote. Actual-vs-plan **[checker]** — q=`flexo ink usage variance calculator`; I=issued/returned/waste/plan; L=net use+variance; O=flag; A=investigate | upstream print-production; exact calculators/vendor formulas; Pack Prep site-fit weak; REJECT |
| 27. Print-cylinder repeat/label count — circumference/teeth/repeat에서 impressions 산정 | label/flexo converter; cylinder/web/label/job changes; Label Roll excluded but converting repeat distinct | Repeat Length **[calculator]** — q=`flexo plate cylinder repeat calculator`; I=teeth,pitch; L=circumference; O=repeat; A=select cylinder. Labels Across/Around **[planner]** — q=`labels per cylinder repeat calculator`; I=repeat,label,gaps,web; L=floor grid; O=up count; A=layout. Gap Reconcile **[checker]** — q=`label repeat gap calculator`; I=repeat,count,label; L=remainder/gap; O=gap; A=adjust. Cylinder Compare **[comparator]** — q=`flexo cylinder teeth repeat chart`; I=candidates,desired repeat; L=error rank; O=closest options; A=proof | direct charts/tools; label-roll/print setup adjacency; REJECT |
| 28. Anilox volume/ink transfer — engraved volume와 coverage를 theoretical ink laydown으로 연결 | flexo technician; anilox/area/coverage changes; current tools none | Theoretical Transfer **[calculator]** — q=`anilox volume ink consumption calculator`; I=BCM/area/transfer%; L=unit conversion; O=volume; A=estimate. Coverage **[calculator]** — q=`anilox ink coverage calculator`; I=anilox volume,coverage,qty; L=deposit×area; O=ink demand; A=stage. Roll Compare **[comparator]** — q=`anilox bcm comparison calculator`; I=roll volumes/user target; L=deviation rank; O=candidates; A=trial. Unit Convert **[converter]** — q=`BCM cm3 m2 converter`; I=BCM or cm³/m²; L=fixed conversion; O=units; A=spec | specialized print-engineering, calculators already exist, safety low but site fit low; REJECT |
| 29. Plate-mounting register/step-repeat — repeat positions와 cumulative error worksheet | flexo prepress/mounter; job/repeat/plate changes; Artwork Preflight excluded but physical mounting math assessed | Step-repeat **[generator]** — q=`flexo step and repeat calculator`; I=web,repeat,lanes,gaps; L=coordinate grid; O=mount coordinates; A=mount. Register Error **[checker]** — q=`flexo registration tolerance calculator`; I=target/observed/user tolerance; L=vector error; O=flags; A=remount. Distortion **[calculator]** — q=`flexo plate distortion calculator`; I=cylinder,plate thickness,repeat; L=supplier formula inputs; O=scale factor; A=prepress. Mount Sheet **[builder]** — q=`flexo plate mounting worksheet`; I=job/colors/coordinates; L=structured record; O=print sheet; A=sign off | artwork/preflight/label print setup overlap and vendor-dependent formulas; REJECT |
| 30. Web-roll remaining length from OD/core — partial roll inventory를 geometry로 추정 | converter/warehouse; OD/core/caliper/material changes; Label Roll excluded but general web inventory still adjacent | Remaining Length **[calculator]** — q=`roll length calculator outside diameter core thickness`; I=OD,core,caliper; L=annulus/caliper; O=length; A=plan job. OD from Length **[calculator]** — q=`roll diameter calculator length thickness`; I=length,core,caliper; L=inverse annulus; O=OD; A=check storage. Weight Reconcile **[checker]** — q=`roll length from weight calculator`; I=gsm,width,net mass; L=mass/(gsm×width); O=length; A=compare OD estimate. Job Sufficiency **[planner]** — q=`remaining roll enough for job calculator`; I=estimated length,job,waste; L=margin; O=remaining/shortage; A=reserve roll | exact free competitors (Avery/HERMA/Filmquest/Calculator Academy); same reciprocal geometry; FINALIST REJECT |
| 31. Rewind-roll build diameter — job length를 finished roll OD/roll count로 변환 | slitter/rewinder; web/core/caliper/job changes; Storage/Label Roll adjacent | Build OD **[calculator]** — q=`rewind roll diameter calculator`; I=length,core,caliper; L=annular winding; O=OD; A=set roll. Max Length **[checker]** — q=`maximum roll length by diameter`; I=max OD,core,caliper; L=inverse; O=length; A=split job. Roll Count **[planner]** — q=`rewind rolls required calculator`; I=job length,max roll length,waste; L=ceil; O=rolls/last length; A=plan cores. Build Compare **[comparator]** — q=`core diameter roll build comparison`; I=core options/job; L=OD deltas; O=table; A=choose | same engine as #30 and direct free tools; MERGE/REJECT |
| 32. Roll-splice timing/runtime — remaining web가 splice point에 도달하는 시간과 planned waste | web press/packer; speed/remaining length/job changes; Changeover excluded but web-specific timer assessed | Time-to-splice **[calculator]** — q=`web roll change time calculator`; I=remaining length,speed; L=length/rate; O=time; A=prepare. Splice Position **[planner]** — q=`roll splice position planner`; I=repeat,remaining,protected window; L=modulo repeat; O=safe candidate positions; A=mark. Splice Waste **[calculator]** — q=`web splice waste calculator`; I=leader/tail/splices; L=sum; O=waste length; A=allow material. Job Finish **[checker]** — q=`will roll finish print job calculator`; I=remaining,job,splices,waste; L=net margin; O=sufficiency; A=stage roll | changeover/order-deadline/roll-length combination; 4th derives from same inventory equation; REJECT |
| 33. Slit-width/trim plan — master web에 slit widths와 edge trim 배치 | converter/planner; master roll/order widths/kerf change; Cut Yield excluded underlying problem | Width Fit **[calculator]** — q=`slitting width calculator`; I=master,slits,trim,knife loss; L=sum/margin; O=fit/trim; A=set knives. Pattern Finder **[planner]** — q=`roll slitting optimization calculator`; I=width demands/qty; L=cutting-stock heuristic; O=patterns; A=plan runs. Knife Positions **[generator]** — q=`slitter knife setup calculator`; I=ordered widths,edge trim; L=cumulative positions; O=setup sheet; A=position knives. Pattern Compare **[comparator]** — q=`slitting trim waste comparison`; I=two patterns; L=waste/change count; O=delta; A=choose | exactly Cut Yield/changeover underlying problem; excluded/MERGE |
| 34. Web-tension/load planning — tension per width와 zone target 기록 | web converting operator; material,width,zone target changes; no current equivalent | Tension/Width **[calculator]** — q=`web tension calculator pli`; I=total tension,width; L=divide/convert; O=PLI/N/cm; A=record target. Taper Profile **[planner]** — q=`rewind taper tension calculator`; I=start tension,user taper,diameter; L=user-defined curve; O=zone targets; A=trial. Load-cell Split **[checker]** — q=`web tension load cell calculation`; I=wrap angle,cell reading,geometry; L=force resolution; O=web tension; A=calibrate. Zone Compare **[comparator]** — q=`web tension zone calculator`; I=unwind/process/rewind readings,targets; L=variance; O=flags; A=inspect | equipment safety/control and specialist engineering; REJECT |
| 35. Coating add-on from wet/dry thickness — coated paper/film의 mass, solids, coverage 환산 | converter/coater; substrate/coating/solids/job changes; material budget differs from physical coating | Dry Coat Weight **[calculator]** — q=`coating weight calculator gsm thickness density`; I=thickness,density,solids; L=volume×density×solids; O=g/m²; A=set trial. Wet Pickup **[calculator]** — q=`wet coating add on calculator`; I=dry target,solids; L=dry/solids; O=wet g/m²; A=stage. Batch Volume **[planner]** — q=`coating consumption calculator area`; I=area,wet add-on,density,waste; L=mass/volume; O=kg/L; A=mix. Actual Add-on **[checker]** — q=`coating add on by basis weight`; I=uncoated/coated sample weights,area,tolerance; L=difference/area; O=g/m²,flag; A=adjust per procedure | Calcimator and Coatings & Adhesive Corp free suites; direct competition; FINALIST REJECT |
| 36. Laminating adhesive mix/pot-life planning — multi-part mix와 batch timing | laminating operator; ratio/job speed/pot life changes; hot-melt differs by reactive mix | Mix Quantity **[calculator]** — q=`laminating adhesive mix ratio calculator`; I=ratio,total batch; L=proportion; O=component masses; A=weigh. Coverage Batch **[planner]** — q=`laminating adhesive consumption calculator`; I=area,coat weight,solids,waste; L=mass; O=batch quantity; A=mix. Pot-life Schedule **[planner]** — q=`adhesive pot life batch size calculator`; I=usage rate,user pot life,reserve; L=max consumable batch; O=batch/time; A=schedule. Mix Reconcile **[checker]** — q=`adhesive mix ratio tolerance checker`; I=actual component weights,target,user tolerance; L=ratio variance; O=flag; A=quarantine/review | chemical handling/supplier instructions; existing free ratio tools; safety risk; REJECT |
| 37. Film gauge/roll-weight reconciliation — film thickness·width·length·density·weight 교차검증 | film buyer/converter; roll/spec/sample changes; Weight/Label Roll differs by film physical reconciliation | Roll Weight **[calculator]** — q=`plastic film roll weight calculator`; I=width,length,gauge,density,core; L=volume×density; O=net/gross mass; A=verify PO. Length from Weight **[calculator]** — q=`film length from roll weight`; I=net mass,width,gauge,density; L=inverse mass; O=length; A=inventory. Gauge Estimate **[calculator]** — q=`film thickness from weight calculator`; I=mass,area,density; L=inverse volume; O=gauge; A=sample check. Spec Reconcile **[checker]** — q=`film roll weight tolerance calculator`; I=label values,observed,target tolerance; L=cross-predict variance; O=flags; A=remeasure | one reciprocal physical engine and free supplier calculators; REJECT |
| 38. Bag-making cut length/web yield — finished bag depth/gusset/seal에서 repeat와 web use | bag converter; bag style/dims/seal/job changes; Industrial Bags excluded and Cut Yield adjacent | Cut Length **[calculator]** — q=`plastic bag cut length calculator`; I=bag height,flap,bottom fold,seals; L=style formula; O=repeat; A=set machine. Web Length **[planner]** — q=`bags per roll calculator cut length`; I=cut length,qty,waste; L=multiply; O=web/job; A=stage roll. Gusset Unfolded Width **[calculator]** — q=`gusset bag web width calculator`; I=finished width,gusset,seams; L=flat-web geometry; O=web width; A=spec. Yield Compare **[comparator]** — q=`bag size material usage comparison`; I=two styles/dims; L=area×qty; O=web delta; A=trial | industrial bags/cut yield and style formulas; excluded/MERGE |
| 39. Perforation repeat/count planning — web repeat 내 perforation 위치와 finished units 수 | converter/packing material maker; repeat/job/skip pattern changes; no current exact tool | Perf Count **[calculator]** — q=`perforation spacing calculator roll`; I=length,pitch,edge offsets; L=floor positions; O=count/remainder; A=set job. Position List **[generator]** — q=`perforation position calculator`; I=repeat,pitch,skip rules; L=coordinate generation; O=positions; A=setup sheet. Tear-off Yield **[planner]** — q=`perforated roll sheets calculator`; I=roll length,sheet repeat,waste; L=floor; O=sheets; A=label roll. Pattern Compare **[comparator]** — q=`perforation repeat comparison`; I=two pitches/job; L=count/remainder; O=yield delta; A=choose | three modes of one repeat division; low packaging-prep fit; REJECT |
| 40. Pouch-converting web repeat/gusset material — flat web와 finished pouch geometry translation | pouch converter; pouch dimensions/gusset/repeat/job changes; Pouch Fill excluded but upstream converting considered | Web Width **[calculator]** — q=`pouch web width calculator`; I=finished width,gussets,seals; L=unfold geometry; O=web width; A=spec. Cut Repeat **[calculator]** — q=`pouch cut length calculator`; I=height,bottom gusset,seals; L=style geometry; O=repeat; A=set job. Material Area **[planner]** — q=`pouch material consumption calculator`; I=web,repeat,qty,waste; L=area/length; O=web demand; A=stage. Layout **[visualizer]** — q=`pouch web layout diagram`; I=dimensions/folds/seals; L=SVG net; O=diagram; A=prototype | Pouch Fill plus dieline/cut-yield adjacency; style-specific engineering; REJECT |

### Rapid screen → Mid 12

- 40개 family를 160개 가설로 시작했고, 명백한 동일-engine 분할, 기존 exclusion과의 underlying overlap, vendor catalog/CAD/live data dependency, 안전·구조 판단, 이미 완성도 높은 exact free tool을 먼저 제거했다.
- Rapid reject 28: CG/manual handling, pick-pluck foam, floor loading, sleeve, telescoping enclosure, caps/plugs, pallet collars, slip sheets, anchoring, skids, molded pulp, thermoform, window patch, ventilation, barrier wrap, WAT, staples, glue tab, paperboard fold, blank feeder, flute direction, flexo ink, cylinder repeat, anilox, plate mounting, web tension, bag making, perforation, pouch converting. 이 중 #23/#33/#38/#40은 기존 Dieline/Cut Yield/Bags/Pouch underlying problem과 MERGE다.
- Mid validation 12: Hot-melt adhesive application, Gift presentation wrap, Package CG/manual handling, Pick-and-pluck foam, Floor-loaded container, Product sleeve/belly band, Nested-product stack, Protective cap/plug allocation, Pallet-collar configuration, Corrugated hand-hole, Web-roll remaining/build, Coating add-on.

| mid family | demand/long-tail | competition | overlap/static/safety | strong Tools | 판정 |
|---|---|---|---|---:|---|
| Hot-melt application | bead size, consumption, dotting, melt rate, refill, output queries가 manufacturer·operator 문서에서 반복 | 개별 calculator/marketing tool은 있으나 neutral end-to-end six-step cluster 없음 | user-entered values, deterministic; 기존 closure diagnosis와 분리; settings/strength를 주장하지 않으면 안전 | **6** | finalist |
| Gift wrap | wrap/ribbon/bag/tissue 자연 query 다수 | CraftCalcs/Hivly가 exact free no-login suite | static이나 free gap 없음 | 4 | finalist |
| CG/manual handling | balance/CG/lift-point query 존재 | engineering calculators/standards | structural/manual-handling safety; packaging search 좁음 | 3 | REJECT |
| Pick-pluck foam | forum/Reddit에서 cavity layout 반복 | case vendors/templates, CAD/manual layout | partitions/cut-yield overlap, protection guarantee 위험 | 3 | REJECT |
| Floor-loaded container | high search breadth, mixed load/20ft/40ft | DimPack3D, 3DContainer, FreightCalcs 등 무료 3D | simple static version이 경쟁보다 약하고 transport safety 포함 | 3 | finalist |
| Sleeve/belly band | template/size/search 반복 | BoxTemplate 등 exact generator | 한 wrap geometry를 네 mode로 분할 | 2 | REJECT |
| Nested stack | stack height/count query가 product docs와 fulfillment에서 존재 | Zonos nesting 기능, standalone exact tools 적음 | static 가능; pitch/height/count가 reciprocal | 3 | finalist |
| Caps/plugs | size selector/vendor support 검색 | vendor catalog selectors | catalog data 없이는 가치 약하고 Kit family와 인접 | 2 | REJECT |
| Pallet collars | height/volume/divider query | supplier guides/calculators | Pallet Height+Partitions의 결합 | 2 | REJECT |
| Corrugated hand-hole | placement/size/edge questions와 ISO/DTS guidance | design articles/standards | structural/ergonomic safety; independent calculator intent 약함 | 2 | finalist |
| Web-roll remaining/build | OD/core/thickness/weight query 매우 강함 | Avery, HERMA, Filmquest, Calculator Academy 무료 tools | reciprocal geometry, Label Roll exclusion과 인접 | 3 | finalist |
| Coating add-on | wet/dry/solids/GSM/coverage query 강함 | Calcimator 및 Coatings & Adhesive Corp가 full suite | static 가능하지만 direct free gap 없음; upstream fit | 4 | finalist |

### Deep finalists 5 — actual Tool SERP / functionality / I→L→O→A

| finalist | Tool별 실제 query와 competitor 기능 | closest existing / distinct I→L→O→A | repeat/static/maintenance/safety | 결정 |
|---|---|---|---|---|
| Hot-melt application | `hot melt bead volume calculator`: Loctite manual은 bead diameter/length→volume와 container/day/year까지 계산 예시. `hot melt adhesive consumption calculator`: PUR Adhesives는 measured consumption 우선, bead geometry 및 unit/hour/year 계산. `hot melt dotting savings calculator`: Hotmelt.com은 tanks/hoses/modules 뒤 application data를 받는 3-step lead-oriented calculator. `hot melt melt rate consumption`: Hot Melt Technologies는 actual usage가 melt rate를 넘지 않아야 하고 tank capacity가 refill frequency를 좌우한다고 설명. `hot melt output monitoring`: Graco는 observed consumption monitoring과 abnormal variance를 설명 | Bead: geometry→mL/g→trial. Batch: measured g/pack→kg/containers→stage. Intermittent: on/off duty→g/pack/kg delta→physical test. Capacity: pack rate×use→kg/h/margin→verify equipment. Refill: starting usable mass/rate→time/events→schedule. Output: timed mass/nozzles→g/min/variance→inspect/retest. Tape Usage/Trial Comparison/Closure guide와 input·logic·output·action이 모두 다름 | SKU/run/pattern/rate/tank/sample마다 반복. Pure JS. Vendor rates/targets are user-entered; formula maintenance low. No temperature/pressure/material recommendation, no bond/closure guarantee | **GO, strong 6** |
| Gift presentation | `gift wrap size calculator`, `ribbon length calculator gift box`, `gift bag size calculator`, `tissue paper calculator`: CraftCalcs가 Gift Bag, Gift Wrap, Ribbon, Box, Tissue, Tag를 무료로 한 suite에서 제공; Hivly와 Calculator Academy도 대응 | wrap/bag/ribbon/tissue I→geometry/yield→cut quantity→prepare는 독립적으로 보이나 Pack Prep의 Material calculators와 일부 인접 | 반복/static/safe는 통과. 그러나 exact free no-login/mobile alternative가 family breadth까지 이미 충족 | **REJECT, strong 4 but free-gap gate fail** |
| Floor-loaded container | `container loading calculator`, `mixed carton container loading`, `20ft 40ft load calculator`, `container weight distribution`: DimPack3D/LandedCost/3DContainer/FreightCalcs가 dimensions, mixed items, rotation, 3D visualization, load plans을 무료 또는 freemium으로 제공 | existing Pallet Fit과 floor volume은 다르지만 simple grid output은 실제 query 기대인 mixed 3D packing보다 현저히 약함; weight-zone action은 safety 책임 | repeat yes; robust bin packing/3D and weight safety increase complexity. Static toy version prohibited | **REJECT, strong 3** |
| Web-roll remaining/build | `roll length calculator OD core thickness`, `roll diameter calculator`, `roll length from weight`, `will roll finish job`: Calculator Academy/Avery/HERMA/Filmquest가 OD/core/caliper and mass variants를 정확히 제공 | Remaining/build/weight are same annulus or mass equation modes; job sufficiency is downstream subtraction. Label Roll already excluded | repeat/static yes, maintenance low, safe. 4 independent URLs fail; free competition exact | **REJECT, strong 3** |
| Coating add-on | `coating weight calculator thickness density`, `wet coating add on calculator solids`, `coating consumption area`, `coating add on basis weight`: Calcimator는 wet/dry thickness, density, solids, coverage 변환; Coatings & Adhesive Corp도 adhesive/coating use and cost calculators 제공 | Dry add-on, wet pickup, batch amount, observed basis-weight variance는 distinct actions이나 site가 converting chemistry로 이동 | repeat/static possible; density/solids user-entered. chemical process wording and direct comprehensive alternatives create maintenance/fit cost | **REJECT, strong 4 but free-gap/site-fit gates fail** |

### Demand evidence, long-tail, competitor inspection, keyword-volume access

- 정확한 keyword volume, GSC, GA4, paid keyword database에는 접근하지 못했고 어떤 volume 숫자도 만들지 않았다. 대신 실제 SERP wording, 서로 다른 manufacturer/technical sources, calculator UI, manuals, community/problem repetition을 조합했다.
- browser에서 실제 기능을 연 competitor:
  - Loctite X manual, **Calculating Adhesive Bead Size** — round bead volume, container consumption, daily/yearly consumption 예시: https://www.loctitex.com/en/manuals/bonding/5.3.2-calculating-adhesive-bead-size/
  - Hot Melt Technologies, **Adhesive Consumption** — usage rate, melt rate capacity, tank/refill relationship: https://www.hotmelt-tech.com/adhesive-consumption/
  - Hotmelt.com, **Hot Melt Stitching and Dotting Savings Calculator** — 3-step inputs, application use/savings output, vendor/quote funnel: https://www.hotmelt.com/pages/dotting-savings-calculator
  - PUR Adhesives, consumption/cost guide — measured use, bead geometry, intermittent application, cost/unit/hour/year: https://pur-adhesives.com/hot-melt-adhesive-consumption-cost-per-unit/
  - Graco, **Precisely Monitor Your Hot Melt Adhesive Dispense** — consumption monitoring and abnormal-use signal: https://www.graco.com/gb/en/in-plant-manufacturing/solutions/articles/precisely-monitor-your-hot-melt-adhesive-dispense.html
  - Henkel, tank refill practice and operating consequences: https://next.henkel-adhesives.com/ch/en/articles/hot-melt-tank-refilling.html
- 대표 auxiliary competitor: CraftCalcs gift suite, BoxTemplate sleeve templates, DimPack3D/LandedCost/3DContainer container-loading tools, Avery/HERMA/Filmquest roll calculators, Calcimator and Coatings & Adhesive Corporation coating calculators. 결과 제목만 보지 않고 가능한 페이지는 inputs/logic/outputs/login/free/vendor-dependency/mobile/workflow breadth를 확인했다.
- selected family의 자연 long-tail branches는 bead diameter/volume, grams per package, run kg/container count, continuous vs dot/stitch, hourly demand/melt rate, tank refill timing, timed output/nozzle flow, consumption variance다. 한 head keyword를 6 Tool demand로 확대하지 않고 각 action query와 supporting source를 따로 확인했다.
- enterprise system 없이 한 SKU/job을 다루는 gap은 명확했다: 기존 자료는 한 계산만 제공하거나 vendor lead form/제품 설명에 묶여 있고, free no-account neutral workflow로 **predict → measure → compare → capacity → refill → verify**를 연결한 cluster는 확인하지 못했다.

### 최종 선정과 구현 범위

- **Final decision: GO — Packaging Adhesive Application.** Minimum 4 Hard Gate를 여유 있게 통과하는 strong independent Tools 6개가 각각 다른 search intent, input, logic, output, next action을 가진다. recurring problem, long-tail breadth, small-operator gap, browser-only determinism, manageable maintenance와 safe scope를 함께 통과했다.
- 구현 public URL 9개:
  - Hub `/adhesive.html`
  - `/tools/adhesive-bead-volume.html`
  - `/tools/adhesive-batch-requirement.html`
  - `/tools/intermittent-bead-savings.html`
  - `/tools/adhesive-melt-rate-capacity.html`
  - `/tools/adhesive-tank-refill.html`
  - `/tools/adhesive-output-calibration.html`
  - Guide `/guides/packaging-hot-melt-setup.html`
  - Reference `/reference/hot-melt-adhesive-planning-terms.html`
- 각 Tool은 clear purpose, labeled inputs/units, input acquisition guidance, validation, deterministic result, interpretation, worked example, limitations, next action, related workflow, Reset을 갖는다. 계산기가 장비를 제어하지 않고 사용자 입력 온도/압력조차 받지 않는다.
- source 변경: `assets/calculators.js`(6 engines/registry), `scripts/generate-site.js`(registry/content/hub/Guide/Reference/relations/SEO/sitemap generation 및 신규 Guide/Reference에만 기존 scoped mobile-card table class 적용), `scripts/verify-calculators.js`(fixtures), `scripts/qa.js`(exact inventory). 생성 결과는 hub+6 Tool+Guide+Reference와 `index.html`, `tools.html`, `guides.html`, `reference.html`, `sitemap.xml`, `llms.txt`다. CSS와 global table layout은 변경하지 않았다.
- Hub는 단순 링크 나열이 아니라 Predict → Measure → Plan → Verify 순서와 Guide/Reference를 연결한다. homepage/tools hub에는 cluster entry만 제한적으로 추가했다. GA4, canonical, breadcrumbs, JSON-LD, navigation/footer current pattern을 재사용했다.
- formula boundaries:
  - round bead: `π(d/2)^2 × length × paths`, inch→cm 후 `1 cm³ = 1 mL`, allowance와 user density 적용.
  - batch: `packs × measured g/pack / 1000 × (1+allowance)`, whole container `ceil`.
  - intermittent: `on/(on+off)` duty cycle; bead cross-section 동일 가정.
  - capacity: `g/pack × packs/hour /1000 × allowance`, entered verified melt-rate와 margin.
  - refill: `(starting fill − reserve)/observed kg/h`, run requirement과 discrete full-refill events.
  - output: `collected g / seconds × 60 / nozzles`, entered target 대비 signed variance와 tolerance flag.
- safety: 결과는 plan/record/flag일 뿐 adhesive selection, temperature, pressure, nozzle, maintenance diagnosis, minimum bead, bond strength, equipment capacity guarantee, closure acceptance, distribution performance를 정하지 않는다. supplier/equipment instructions와 controlled physical trial로 다음 action을 명시한다.

### Automated QA 및 local browser QA

- bundled Node만 사용했고 설치/환경 재설정은 하지 않았다.
- `scripts/generate-site.js` regenerate 후 `node --check` 대상 generator/calculator/site/workflow/QA scripts PASS.
- `scripts/qa.js`: **PASS — 85 HTML, 84 sitemap URLs, 7 JavaScript files**. 내부 links/IDs/title/meta/H1/canonical/robots/sitemap/llms/structured data/GA4 포함.
- `scripts/qa.js` 내 content QA: **PASS — 42 calculators, 4 workflow tools, 15 guides, 13 references; duplicate long paragraphs 0; duplicate long sentences 0**.
- `scripts/qa.js` 내 responsive table QA: **PASS — 42 calculator input-definition tables** scoped mobile override.
- `scripts/verify-calculators.js`: **PASS — 42 calculators, 218 independent checks**. 신규 6개 각각 normal, alternate/boundary, impossible/error, deterministic를 포함하고 capacity exceed와 output variance signal을 별도 assertion했다.
- `scripts/verify-workflow-tools.js`: **PASS — 46 normal/boundary/error/safety/deterministic checks**.
- 실제 browser interaction at 390px: 6/6 Calculate 성공, output은 각각 `10.42 mL per pack`, `6.221 kg for the run`, `2.133 g per pack`, `1.584 kg/h required`, `6 h to reserve`, `72 g/min per nozzle`; `data-state=ready`, form error 0, horizontal overflow 0, console error 0. Output Calibration의 collection time=0은 `Collection time must be greater than zero.`와 error state를 내고 Reset은 기본 20초/idle/result placeholder/error blank로 복구했다.
- 신규 9 pages 전부 1440/1280/1024/768/390 = 45 combinations: H1 정상, horizontal overflow 0, visible viewport escape 0, console error 0. 390px에서 Hub와 Tool을 full-page/viewport로 육안 확인했다. 첫 육안 검사에서 신규 Guide/Reference의 일반 `.content-table`이 `tbody th` 28% 규칙 때문에 제목 옆 빈 공간을 남기는 것을 발견했고, generator에서 두 신규 slug에만 기존 `decision-guide-table`/`workflow-record-table` class를 적용했다. 재생성 후 두 표의 모든 `th`/`td`는 각각 `display:block`, 폭 353px(390 viewport content width), 같은 left 11px로 쌓였고 cards/inputs/buttons/result/text wrap/table에 clipping·overlap·overflow가 없었다. 1px clipped `thead`는 접근성용 visually-hidden pattern이다. 모바일 Menu도 `aria-expanded false→true`, nav `data-open=true`, console warning/error 0으로 동작했다.
- 기존 대표 7 pages (`/`, `/tools.html`, Dimensional Weight, Quality hub, Pack Instruction hub, Packaging Trial Guide, Packaging Quality Metrics Reference) × 5 viewports = 35 combinations: H1 정상, horizontal overflow 0, visible escape 0, console error 0. Packaging Trial responsive Decision Guide도 390px card structure 유지.
- 추가 회귀 대상인 Pallet Utilization, Shipping Damage Rate, Carton Count, Pack Instruction Guide는 automated exact fixtures/content/responsive QA로 PASS이며 공통 CSS/workflow JS를 변경하지 않았다.
- homepage user-managed badge block은 generator 전/후 base와 whitespace-normalized exact semantic comparison **true**. 5개, footer 다음 위치, HTML/href/image/order가 모두 보존됐다: KittyLaunch → Sell With Boost → Twelve Tools → Findly.tools → BoostDomainRating. 외부 badge images가 localhost 브라우저에서 5개 모두 로드 차단/실패했지만 source block은 baseline과 동일하고 production asset 변경 대상이 아니다.
